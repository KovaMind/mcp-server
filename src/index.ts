...
server.tool(
  ...
  {
    label: z.string().describe("Name for this secret (e.g., 'aws-staging-key')"),
    value: z.string().describe("The secret value to encrypt and store"),
    tags: z.string().optional().describe("Comma-separated tags for organization"),
    agent_id: z.string().optional().describe("Agent ID (defaults to KOVAMIND_USER_ID env var)"),
  },
  async ({ label, value, tags, agent_id }) => {
    const uid = resolveUserId(agent_id);
    if (!uid) {
      return {
    ...
    try {
      const body: Record<string, unknown> = { agent_id: uid, label, value };
      if (tags) body.tags = tags;
      const data = await apiRequest("POST", "/vault/secrets", body);
      return {
        content: [{ type: "text" as const, text: `Secret "${label}" stored (id: ${data.id}).` }],
      };
    } catch (err: any) {
      return {
        content: [{ type: "text" as const, text: `Vault store failed: ${err.message?.replace(/https?:\/\/[^\s]+/g, "[redacted]").slice(0, 200)}` }],
      };
    }
  }
);
...
server.tool(
  ...
  {
    secret_id: z.string().describe("The secret ID to retrieve"),
    agent_id: z.string().optional().describe("Agent ID (defaults to KOVAMIND_USER_ID env var)"),
  },
  async ({ secret_id, agent_id }) => {
    const uid = resolveUserId(agent_id);
    if (!uid) {
      return {
        content: [{ type: "text" as const, text: "Error: agent_id is required." }],
    ...
    try {
      const data = await apiRequest(
        "GET",
        `/vault/secrets/${encodeURIComponent(secret_id)}?agent_id=${encodeURIComponent(uid)}`
      );
      return {
        content: [{ type: "text" as const, text: `Secret value: ${data.value}` }],
      };
    } catch (err: any) {
      return {
        content: [{ type: "text" as const, text: `Vault get failed: ${err.message?.replace(/https?:\/\/[^\s]+/g, "[redacted]").slice(0, 200)}` }],
      };
    }
  }
);
...
server.tool(
  ...
  async ({ agent_id }) => {
    ...
    try {
      const data = await apiRequest(
        "GET",
        `/vault/secrets?agent_id=${encodeURIComponent(uid)}`
      );
      const secrets = (data.secrets ?? []) as Array<{ id: string; label: string; tags?: string }>;
      if (secrets.length === 0) {
        return { content: [{ type: "text" as const, text: "Vault is empty." }] };
      }
      const lines = secrets.map((s) => `- ${s.label} (id: ${s.id}${s.tags ? `, tags: ${s.tags}` : ""})`);
      return {
        content: [{ type: "text" as const, text: `${secrets.length} secret(s):\n${lines.join("\n")}` }],
      };
    } catch (err: any) {
      return {
        content: [{ type: "text" as const, text: `Vault list failed: ${err.message?.replace(/https?:\/\/[^\s]+/g, "[redacted]").slice(0, 200)}` }],
      };
    }
  }
);
...
server.tool(
  ...
  {
    secret_id: z.string().describe("The secret ID to destroy"),
    agent_id: z.string().optional().describe("Agent ID (defaults to KOVAMIND_USER_ID env var)"),
  },
  async ({ secret_id, agent_id }) => {
    const uid = resolveUserId(agent_id);
    ...
    try {
      ...
      return {
        content: [{
          type: "text" as const,
          text: data.destroyed
            ? `Secret ${secret_id} destroyed.`
            : `Secret ${secret_id} not found.`,
        }],
      };
    } catch (err: any) {
      return {
        content: [{ type: "text" as const, text: `Vault delete failed: ${err.message?.replace(/https?:\/\/[^\s]+/g, "[redacted]").slice(0, 200)}` }],
      };
    }
  }
);
